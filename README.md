...
基于 Claw Code 架构的专业编码助手：
- **系统提示词构建**: 自动发现项目上下文、CLAW.md 指令文件
- **多语言代码执行**: Python、JavaScript、Shell 等
- **代码静态分析**: 语法检查、复杂度分析、代码风格
- **代码审查**: 安全漏洞检测、最佳实践建议
- **Git 操作**: 状态查询、提交、分支管理
...
移植自 OpenKiwi 的核心功能：
- **验证码提取**: 自动识别通知中的验证码并复制到剪贴板
- **通知分类**: 智能评估通知重要性，提取关键词和实体
- **自动回复**: 白名单管理、速率限制、回复模板
- **代码沙箱**: 危险命令检测、超时控制、输出截断
- **手势执行器**: 低延迟手势执行（50-300ms）、节点缓存（TTL 500ms）
